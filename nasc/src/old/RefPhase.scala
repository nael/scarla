...
class RefPhase extends Phase[CompilationUnit, CompilationUnit] {
  ...
  def removeRefTypes(st: Tree): Tree = st match {
    case e: Expr => { e.ty = Types.Qual.removeAttribute(e.ty, Types.Attributes.Ref); e }
    case _ => st
  }

  def simplifyRefDeref(st: Tree): Tree = st match {
    case PtrRef(PtrDeref(u)) => u
    case PtrDeref(PtrRef(u)) => u
    case _ => st
  }

  def transformCalls(st: Tree): Tree = st match {
    case call: Call => {
  ...
  def transformDefs(st: Tree): Tree = st match {
